...
@interface SOFMAudioPalyer ()
{
    ...
    SOFMPacketQueue             *audioPacketQueue;
    AVFrame                     *pAudioFrame;
    SwrContext                  *pSwrCtx;
...
@implementation SOFMAudioPalyer
-(instancetype)init{
    self = [super init];
    if (self) {
        _networkWay =  kTCP;
        _playState  = eAudioStop;
    }
    return self;
}
-(void)playFFmpegAudioStreamURL:(NSString*)mmsURL withTransFerWay:(kNetworkWay)networkWay comeBackErrorBlock:(void (^)(NSString *))errorBlcok{
    
    self.playState = eAudioTransiting;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        //监测是否被ffmpeg支持播放
        if ([self initFFmpegAudioStream:mmsURL withTransferWay:networkWay] == NO) {
            NSLog(@"Init ffmpeg failed");
            errorBlcok (@"Init ffmpeg failed 1");
            self.playState = eAudioStop;
            return;
        }
        //音频流开空间
        if (![self initPacketQueue:nil withCodecCtx:pAudioCodeCtx]) {
            errorBlcok (@"Init ffmpeg failed 2");
            self.playState = eAudioStop;
            return;
        }
//        dispatch_async(dispatch_get_main_queue(), ^{
//            sleep(5);
            if ([self getStatus] != eAudioPlaying) { //没有播放就播放
                [self play];
                self.playState = eAudioPlaying;
                errorBlcok (nil);
            } else {
               errorBlcok (@"play status error!");
                self.playState = eAudioStop;
            }
//        });
        // Read Packet in another thread，读取音频流
...
void HandleOutputBuffer(void *aqData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer)
{
    ...
    SOFMAudioPalyer *player = (__bridge SOFMAudioPalyer *)aqData;
    [player putAVPacketsIntoAudioQueue:inBuffer];
}

- (BOOL)initPacketQueue:(SOFMPacketQueue *)audioQueue withCodecCtx:(AVCodecContext *)pAudioCodecCtx
{
    int i = 0;
    int audio_index = 1;
    int vBufferSize = 0;
    ...
    if (audioQueue) {
        ...
        audioPacketQueue = [[SOFMPacketQueue alloc] initWithQueue];
    }
...
- (void)decodeAudioFile:(NSString *)filePathIn toPCMFile:(NSString *)filePathOut withCodecCtx:(AVCodecContext *)pAudioCodecCtx withFormat:(AVFormatContext *)pFormatCtx withStreamIdx:(int)audioStream
{
    ...
    [SOFMMPEGUtilities writeWAVHeaderWithCodecCtx:pAudioCodecCtx withFormatCtx:pFormatCtx toFile:wavFile];
...
- (BOOL)initFFmpegAudioStream:(NSString *)filePath withTransferWay:(kNetworkWay)network
{
    ...
    // Parse header ADTS 头格式较验
    uint8_t pInput[] = {0x0ff,0x0f9,0x058,0x80,0,0x1f,0xfc};
    tAACADTSHeaderInfo vxADTSHeader={0};
    BOOL isADTS = [SOFMMPEGUtilities parseAACADTSHeader:pInput toHeader:(tAACADTSHeaderInfo *) &vxADTSHeader];
    if (isADTS) {
        NSLog(@"ADTS 头格式较验成功");
    } else {
        NSLog(@"ADTS 头格式较验失败");
    ...
    if (strncmp([filePath UTF8String], "rtsp", 4) == 0) {
        ...
        isLocalFile = NO;
    } else if ((strncmp([filePath UTF8String], "mmsh:", 5) == 0) ||(strncmp([filePath UTF8String], "http:", 5) == 0)) {
        pAudioInPath = filePath;
        isLocalFile = NO;
    }else {
