...
@implementation SOFMMPEGUtilities
...
//    uint8_t pInput[] = {0x0ff,0x0f9,0x058,0x80,0,0x1f,0xfc};
    ...
    ADTSHeader->ID = (input[1]&0x08) >> 4;    //1111 1001
                                              //0000 1000  －－》0000 1000－－》0000 0001
    ...
    ADTSHeader->sampling_frequency_index = (input[2]&0x3C) >> 2;   //0x3c :(input[2]&0x3C) >> 2)   //0x4: 24000 Hz(((input[2]&0x3C) >> 2)&0x04)
...
+ (id)initForDecodeAudioFile:(NSString *)filePathIn toPCMFile:(NSString *)filePathOutput
{
    ...
    [SOFMMPEGUtilities writeWAVHeaderWithCodecCtx:pAudioCodeCtx withFormatCtx:pAudioFormatCtx toFile:wavFile];
