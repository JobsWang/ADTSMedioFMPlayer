...
-(void)playFFmpegAudioStreamURL:(NSString*)mmsURL withTransFerWay:(kNetworkWay)networkWay comeBackErrorBlock:(void (^)(NSString *))errorBlcok{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([self initFFmpegAudioStream:mmsURL withTransferWay:networkWay] == NO) {
            ...
            errorBlcok (@"Init ffmpeg failed");
            return;
        }
        //音频流开空间
        [self initPacketQueue:nil withCodecCtx:pAudioCodeCtx];
        dispatch_async(dispatch_get_main_queue(), ^{
            sleep(5);
            if ([self getStatus] != eAudioRunning) { //没有播放就播放
                [self play];
                errorBlcok (nil);
            } else {
...
- (void)initPacketQueue:(SOFMPacketQueue *)audioQueue withCodecCtx:(AVCodecContext *)pAudioCodecCtx
{
    int i = 0;
    int audio_index = 1;
    int vBufferSize = 0;
...
- (BOOL)initFFmpegAudioStream:(NSString *)filePath withTransferWay:(kNetworkWay)network
{
    ...
    tAACADTSHeaderInfo vxADTSHeader={0};
    BOOL isADTS = [AudioUtilities parseAACADTSHeader:pInput toHeader:(tAACADTSHeaderInfo *) &vxADTSHeader];
    if (isADTS) {
        NSLog(@"ADTS 头格式较验成功");
    } else {
        NSLog(@"ADTS 头格式较验失败");
